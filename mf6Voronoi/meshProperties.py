...
class meshShape:
    ...
    def get_gridprops_disv(self, intervalNumber=10,save_spatial_index=True, verbose=False):
        ...
        # 1. Extract coordinates and count per polygon
        allVerticesList = self.mesh_df.get_coordinates().values
        polyLenVerticies = self.mesh_df.count_coordinates().values  # numpy array

        # 2. Deduplicate vertices across the entire grid
        uniqueVerticesArray, uinv = np.unique(allVerticesList, axis=0, return_inverse=True)
        uniqueVerticesList = uniqueVerticesArray.tolist()
    
        if verbose:
            print('\nExtracting cell centroids...')

        # 3. Fast centroid extraction
        centroids_series = self.mesh_df.geometry.centroid
        centroids_arr = np.column_stack([centroids_series.x.to_numpy(), centroids_series.y.to_numpy()])

        if verbose:
            print('\nBuilding cell2d array structure...')

        # 4. OPTIMIZED CELL2D CONSTRUCTION:
        # Split the flattened inverse vertex indices into sub-arrays for each polygon
        cumul_splits = np.cumsum(polyLenVerticies)[:-1]
        grouped_uinv = np.split(uinv, cumul_splits)

        # Fast list assembly for MODFLOW DISV cell2d structure
        # Structure: [cell_index, x_centroid, y_centroid, num_vertices, v1, v2, v3, ...]
        cell2dArrays = [
            [i, float(cx), float(cy), int(nvert)] + poly_vids.tolist()
            for i, ((cx, cy), nvert, poly_vids) in enumerate(zip(centroids_arr, polyLenVerticies, grouped_uinv))
        ]
        ...
        self.disvDict['centroids'] = centroids_arr.tolist()
